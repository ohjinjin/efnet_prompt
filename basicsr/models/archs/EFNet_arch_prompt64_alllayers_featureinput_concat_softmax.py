...
#                 img.save(f"/home/work/data/code/EFNet_original/EFNet/experiments/result_weight_nhwc_dilation/{counter}/{each_batch}/prompt_weight_{_}.png")
        ...
        ######################
        counter = 0
        while os.path.exists(f"/home/work/data/code/EFNet_original/EFNet/experiments/result_weight_nhwc_dilation_prompt64_concat_softmax/{counter}"):
            counter += 1
        ######################
        # counter = None
...
#         print("CHEKKKKBBBBCCCCCCCCNNNNNHHHHWWWWWWWW::",prompt_weights[0,:,0].shape)
            ...
            for each_level in range(3):
                ...
                if self.stride_promptweight==(2**each_level):
                    os.makedirs(f"/home/work/data/code/EFNet_original/EFNet/experiments/result_weight_nhwc_dilation_prompt64_concat_softmax/{counter}/{each_level}/", exist_ok=True)
                    print(f"WEIGHTMAP::: /home/work/data/code/EFNet_original/EFNet/experiments/result_weight_nhwc_dilation_prompt64_concat_softmax/{counter}/{each_level}/")
                    # 텐서를 PIL 이미지로 변환
        ...
        #                 print("CHECKJJINJIN:::::::", img.shape)
                        img = (img * 255).astype('uint8')  # 그레이스케일 값 범위를 0-255로 조정
                        img = Image.fromarray(img)
                        img.save(f"/home/work/data/code/EFNet_original/EFNet/experiments/result_weight_nhwc_dilation_prompt64_concat_softmax/{counter}/{each_level}/prompt_weight_{_}.png")
            ######################
