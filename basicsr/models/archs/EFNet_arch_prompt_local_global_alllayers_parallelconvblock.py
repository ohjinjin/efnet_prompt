...
import math
from basicsr.models.archs.arch_util import EventImage_w_promptLG_ChannelAttentionTransformerBlock, PromptMapGenBlock, PromptMapGenBlock1D
...
class GlobalPromptDist(nn.Module):
    """
    입력:  (B, C_in, H, W)
    출력:  (B, prompt_num)  # global softmax → 각 scene 전체에 대한 채널합=1
    """
    def __init__(self, in_channels: int, prompt_num: int, relu_slope: float = 0.2):
        super().__init__()
        self.prompt_num = prompt_num
       ...
       # 전역 특징 뽑기: GAP으로 H,W → 1,1
        self.pool = nn.AdaptiveAvgPool2d((1, 1))

        # 분포를 위한 선형 변환
        self.fc = nn.Linear(512, prompt_num)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        feat = self.backbone(x)               # (B, 512, H, W)
        pooled = self.pool(feat).view(x.size(0), -1)  # (B, 512)

        logits = self.fc(pooled)              # (B, P)

        # 전역 softmax (배치별 Σ=1)
        logits = logits - logits.max(dim=1, keepdim=True).values
        probs = F.softmax(logits, dim=1)      # (B, P)
...
class EFNet(nn.Module):
    def __init__(self, in_chn=3, ev_chn=6, wf=64, depth=3, fuse_before_downsample=True, relu_slope=0.2, num_heads=[1,2,4]):
        ...
        self.prompt_global_weight = GlobalPromptDist(in_channels=ev_chn+in_chn, prompt_num=self.prompt_num, relu_slope=relu_slope)
    ...
    def forward(self, x, event, mask=None):
        ...
        prompt_weight_map_global = self.prompt_global_weight(torch.cat([event, image], dim=1))# prompt_weight_map.mean(dim=[2,3])
        ...
        for i, down in enumerate(self.down_path_1):
            if (i+1) < self.depth:
                ...
                x1, x1_up = down(x1, prompt_weight_map, prompt_weight_map_global, event_filter=ev[i], merge_before_downsample=self.fuse_before_downsample)
                encs.append(x1_up)
                ...
                x1 = down(x1, prompt_weight_map, prompt_weight_map_global, event_filter=ev[i], merge_before_downsample=self.fuse_before_downsample)
...
class UNetConvBlock(nn.Module):
    def __init__(self, in_size, out_size, downsample, relu_slope, prompt_num=None, stride_promptweight=None, use_emgc=False, num_heads=None): # cat
        ...
        if self.num_heads is not None:
            ...
            self.downsample_prompt_global = conv_down(out_size, out_size, bias=False)
            self.image_event_transformer = EventImage_w_promptLG_ChannelAttentionTransformerBlock(out_size, num_heads=self.num_heads, ffn_expansion_factor=4, bias=False, LayerNorm_type='WithBias')
    ...
    def forward(self, x, prompt_weight_map=None, prompt_weight_globalmap=None, enc=None, dec=None, mask=None, event_filter=None, merge_before_downsample=True):
        out = self.conv_1(x)
        ...
        if event_filter is not None and merge_before_downsample:
            ...
            prompt_global = self.prompt_globalblock(prompt_weight_globalmap, prompt_local.shape[2], prompt_local.shape[3])
            out = self.image_event_transformer(out, event_filter, prompt_local, prompt_global) 
             
        if self.downsample:
            out_down = self.downsample(out)
            if not merge_before_downsample: 
                out_prompt_down = self.downsample_prompt(prompt_local)
                out_prompt_global_down = self.downsample_prompt_global(prompt_global)
                out_down = self.image_event_transformer(out_down, event_filter, out_prompt_down, out_prompt_global_down) 
