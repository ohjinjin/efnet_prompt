...
import math
from basicsr.models.archs.arch_util import EdgeAwareSharpening_ChannelAttentionTransformerBlock, PromptGuided_ChannelAttentionTransformerBlock, PromptMapGenBlock, PromptMapGenBlock1D
...
class EFNet(nn.Module):
    ...
    def forward(self, x, event, mask=None):
        image = x
        edge = torch.cat([event[:,2,:,:].unsqueeze(1), event[:,3,:,:].unsqueeze(1)], dim=1)
        motion = torch.cat([event[:,0,:,:].unsqueeze(1), event[:,-1,:,:].unsqueeze(1)], dim=1)
        # B, C, H, W = event.shape
...
#                 img.save(f"/home/work/data/code/EFNet_original/EFNet/experiments/result_weight_nhwc_dilation/{counter}/{each_batch}/prompt_weight_{_}.png")
        ...
        for i, down in enumerate(self.down_path_1):
            if (i+1) < self.depth:
                ...
                x1, x1_up = down(x1, event_filter=ev[i], edge=edge, motion=motion, merge_before_downsample=self.fuse_before_downsample, counter=counter)
                encs.append(x1_up)
                ...
                x1 = down(x1, event_filter=ev[i], edge=edge, motion=motion, merge_before_downsample=self.fuse_before_downsample, counter=counter)
...
class UNetConvBlock(nn.Module):
    def __init__(self, in_size, out_size, downsample, relu_slope, prompt_num=None, stride_promptweight=None, use_emgc=False, num_heads=None): # cat
        ...
        if self.num_heads is not None:
            ...
            self.conv_edge = nn.Conv2d(2, out_size, kernel_size=3, padding=1, stride=stride_promptweight, bias=True)
            self.downsample_edge = conv_down(out_size, out_size, bias=False)
            self.conv1d_edge = nn.Conv2d(out_size * 2, out_size, kernel_size=1)
            self.conv_motion = nn.Conv2d(2, out_size, kernel_size=3, padding=1, stride=stride_promptweight, bias=True)
            self.downsample_motion = conv_down(out_size, out_size, bias=False)
            self.conv1d_motion = nn.Conv2d(out_size * 3, out_size, kernel_size=1)
            self.edgd_module = EdgeAwareSharpening_ChannelAttentionTransformerBlock(out_size, num_heads=self.num_heads, ffn_expansion_factor=4, bias=False, LayerNorm_type='WithBias')
            ...
            # self.prompt_weight = PixelwisePromptDist(in_channels=3*out_size, prompt_num=self.prompt_num, relu_slope=relu_slope)
            self.prompt_weight = ResNet18PromptDist(in_channels=3*out_size, prompt_num=self.prompt_num, pretrained=False, temperature=1.0)
    ...
    def forward(self, x, edge=None, motion=None, enc=None, dec=None, mask=None, event_filter=None, merge_before_downsample=True, counter=0):
        out = self.conv_1(x)
        ...
        if event_filter is not None and merge_before_downsample:
            ...
            out = self.edgd_module(self.conv1d_edge(torch.cat([out, out_edge], 1)), event_filter)
            out_motion = self.conv_motion(motion)
            prompt_weight_map = self.prompt_weight(torch.cat([out, event_filter, out_motion], dim=1))
...
#                         img.save(f"/home/work/data/code/EFNet_original/EFNet/experiments/result_weight_nhwc_dilation_prompt6_softmax/{counter}/{each_level}/prompt_weight_{_}.png")
            ...
            out = self.image_event_transformer(self.conv1d_motion(torch.cat([out, prompt_local, prompt_global], 1)), event_filter) 
             
        if self.downsample:
            out_down = self.downsample(out)
            if not merge_before_downsample:
                out_edge_down = self.downsample_edge(out_edge)
                out_down = self.edgd_module(self.conv1d_edge(torch.cat([out_down, out_edge_down], 1)), event_filter)
                out_motion_down = self.downsample_motion(out_motion)
                prompt_weight_map_down = self.prompt_weight(torch.cat([out_down, event_filter, out_motion_down], dim=1))
                prompt_weight_map_global_down = prompt_weight_map_down.mean(dim=[2,3])
                ######################
    ...
    #                         img.save(f"/home/work/data/code/EFNet_original/EFNet/experiments/result_weight_nhwc_dilation_prompt6_softmax/{counter}/{each_level}/prompt_weight_{_}.png")
                ...
                prompt_global_down = self.prompt_globalblock(prompt_weight_map_global_down, prompt_local_down.shape[2], prompt_local_down.shape[3])
                # print(":::::::::::prompt_local.shape", prompt_local.shape)
                # prompt_local_down = self.downsample_prompt(prompt_local)
                out_down = self.image_event_transformer(self.conv1d_motion(torch.cat([out_down, prompt_local_down, prompt_global_down], 1)), event_filter) 
